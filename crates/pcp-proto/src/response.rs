...
pub enum ErrorCode {
    ...
    /// server.
    #[error("sent version is not supported")]
    UnsuppVersion = 1,
    ...
    #[error("option could not be parsed")]
    MalformedOption = 6,
    ...
    #[error("spurious network failure")]
    NetworkFailure = 7,
    ...
    #[error("not enough resources for this request")]
    NoResources = 8,
...
/// The error that can occur when working with the PCP protocol response.
#[derive(Debug, thiserror::Error, PartialEq, Eq)]
pub enum Error {
    /// Decoding failed.
    #[error(transparent)]
    DecodeError(DecodeError),
    /// Error code was communicated to us.
    #[error(transparent)]
    ErrorCode(ErrorCode),
}
